#include <bits/stdc++.h>
using namespace std;
// Function to encrypt the message using transposition keyless cipher
//combined
...
string encrypt(string message,int col) {
    ...
    // Read the characters from the mattrix column-wise
    string encrypted = "";
    for (int j = 0; j < col; j++) {
        for (int i = 0; i < rows; i++) {
...
int main() {
    ...
    cout << "Encrypted message after first encryption: " << encrypted << endl;
    encrypted=encrypt(encrypted,col);
    cout << "Encrypted message after second encryption: " << encrypted << endl;
    
     encrypted = encrypt1(encrypted,key);
    
    cout << "Encrypted message after third encryption: " << encrypted << endl;
    encrypted=encrypt(encrypted,col);
    cout << "Encrypted message after fourth encryption: " << encrypted << endl;
   ...
   string decrypted = decrypt(encrypted,col);
    cout << "Decrypted message after first decryption: " << decrypted << endl;
     ...
     decrypted = decrypt(decrypted,col);
     cout << "Decrypted message after third decryption: " << decrypted << endl;
      decrypted = decrypt1(decrypted,key);
      cout << "Decrypted message after fourth decryption: " << decrypted << endl;
